...
def compute_satisfaction(user):
    ...
    if(distance < 0.35):
        satisfaction = 5
    elif(distance > 0.35 and distance < 0.45):
        satisfaction = 4
    elif(distance > 0.45 and distance < 0.55):
        satisfaction = 3
    elif(distance > 0.55 and distance < 0.7):
        satisfaction = 2
    elif(distance > 0.7 and distance < 1):
        satisfaction = 1
    else:
        satisfaction = 0
...
if __name__ == '__main__':     
    ...
    user_antonella = user.User("Antonella", [], sample_route, "red")
    user_spyna = user.User("Spyna", [], sample_route, "green")
    
    users = [user_maria_anna, user_antonella, user_spyna]
    
    for user in users:
        print(f"Hello, {user.name}! Welcome in Norba!")
        ...
        suggested_canvas, examinated_canvas = get_next_canvas(choiced_canvas, sample_route, actual_pearl, user.canvas_in_pearl_counter[actual_pearl], user, 0)
        # pl.plot_point(choiced_canvas, examinated_canvas, user, suggested_canvas, sample_route, actual_pearl)
        print_canvas_wp(suggested_canvas)
        
        choice = ''
        while len(user.visited_canvas) < total_canvas:
            while choice == '':
            ...
            # pl.plot_point(choiced_canvas, examinated_canvas, user, suggested_canvas, sample_route, actual_pearl)
            print_canvas_wp(suggested_canvas)
            print(f"\nWatched {user.canvas_in_pearl_counter} of {get_number_of_canvas_from_pearl(sample_route[actual_pearl])} canvas. Total canvas are: {total_canvas}")    
            choice = ''

        print("End of the route! Byeee :3")

    pl.plot_satisfaction_users(users)